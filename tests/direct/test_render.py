...
def assets(tmp_path):
    ...
    command(
        ...
        "lavfi",
        "-i",
        "sine=frequency=220:sample_rate=32000",
        "-t",
        "0.4",
        str(music),
    )
    ...
    return {
        key: {"path": str(path), "kind": kind, "metadata": probe(path)}
...
def test_mixed_sources_speed_titles_captions_overlays_music(assets, tmp_path):
    plan = {
        ...
        "overlays": [
            {"asset_id": "still", "start": 0.8, "duration": 0.3, "layout": "full"},
            {"asset_id": "red", "start": 1.15, "duration": 0.15, "layout": "pip"},
        ],
        "captions": True,
        "music": {"asset_id": "music", "volume": 0.1, "duck": True},
    }
    transcripts = {
        "blue": {
            "words": [{"text": "CAPTION", "start": 0.2, "end": 0.4, "type": "word"}]
        }
    }
...
def test_word_timing_maps_trim_speed_and_ignores_spacing():
    plan = {
        "cuts": [
            {"asset_id": "a", "start": 1, "end": 3, "speed": 2, "volume": 1},
    ...
    words = {
        "a": {
            "words": [
                {"text": "trimmed", "start": 0.8, "end": 1.4},
...
def test_narration_is_audible_only_inside_its_finite_timeline_window(assets, tmp_path):
    import array
    ...
    assert max(abs(sample) for sample in during) > 500
    assert max(abs(sample) for sample in after) < 10
    assert float(probe(output)["format"]["duration"]) == pytest.approx(1.2, abs=0.08)


def test_narration_caption_timing_maps_source_window_to_timeline():
    plan = {
        "cuts": [
            {"asset_id": "picture", "start": 0, "end": 2, "speed": 1, "volume": 0}
        ],
        "narration": {
            "asset_id": "voice",
            "source_start": 1,
    ...
    transcripts = {
        "voice": {
            "words": [{"text": "VOICE", "start": 1.2, "end": 1.5, "type": "word"}]
        }
    }
    assert _caption_events(plan, transcripts) == [
        {
            "text": "VOICE",
