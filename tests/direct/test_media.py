...
def test_public_target_rejects_private_and_mixed_dns(monkeypatch):
    ...
    assert public_target("https://example.com/video.mp4?x=1") == (
        "example.com", "8.8.8.8", "/video.mp4?x=1")
    for url in ["http://example.com/a.mp4", "https://user:pass@example.com/a.mp4",
                "file:///tmp/video.mp4", "https://example.com:8080/a.mp4"]:
...
def test_download_enforces_stream_limit(monkeypatch, tmp_path):
    ...
    class Response:
        ...
        def read(self, n): return b"0123456789"
    class Connection:
