...
_EDL_PROMPT = (
    ...
    "transition}}], titles[{{text, start, end, position}}], subtitles(bool), "
    "music. Cut filler/dead-air, tighten, and order to match the style. Times "
    "are seconds.\n\n"
    "TITLES: add a title only where it earns its place (an opener, a key name/"
    "stat, a payoff) — text is the words on screen, start/end are timeline "
    "seconds, position is top|center|bottom. They are burned onto the render, "
    "so keep them short and legible; omit when the footage speaks for itself.\n\n"
    "{effects}\n\n"
    "STYLE:\n{style}\n\nMANIFEST:\n{manifest}\n\nTRANSCRIPT:\n{transcript}\n\n"
    "INTENT:\n{prompt}\n{feedback}"
)
