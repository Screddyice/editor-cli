...
def _video(preview: bool) -> list[str]:
    return [
        ...
        "libx264",
        "-preset",
        "ultrafast" if preview else "fast",
        "-crf",
...
def _caption_events(plan: dict, transcripts: dict) -> list[dict]:
    ...
    def append_words(
        ...
        for word in transcripts.get(asset_id, {}).get("words", []):
            ...
            try:
                ...
                raise ValueError(
                    "Caption words require numeric start/end timings"
                ) from None
            if not math.isfinite(start) or not math.isfinite(end) or end < start:
                raise ValueError("Caption word timing is invalid")
            start, end = max(start, source_start), min(end, source_end)
            ...
            if start < end and text:
                ...
                events.append(
                    {
                        "text": text,
                        "start": timeline_start + (start - source_start) / speed,
    ...
    for cut in plan["cuts"]:
        if cut["volume"] > 0:
            append_words(
                cut["asset_id"], cut["start"], cut["end"], offset, cut["speed"]
            )
...
def _draw_text(canvas: Image.Image, event: dict, caption: bool = False) -> None:
    ...
    while True:
        ...
        bounds = draw.multiline_textbbox(
            (0, 0), text, font=font, spacing=4, stroke_width=2
        )
    ...
    top = {
        ...
        "bottom": height * (0.76 if caption else 0.88) - text_height,
    }[event["position"]]
    draw.multiline_text(
        ((width - (bounds[2] - bounds[0])) / 2, top - bounds[1]),
        text,
        font=font,
        fill="white",
        stroke_width=2,
...
def _text_layer(
    ...
    events = [(event, False) for event in titles] + [
        (event, True) for event in captions
    ]
    for index, (event, _) in enumerate(events):
        start = max(0, math.ceil(event["start"] * fps - 1e-7))
        end = min(
            total_frames, math.ceil((event["start"] + event["duration"]) * fps - 1e-7)
        )
        if end > start:
...
def render(
    ...
    for index, cut in enumerate(plan["cuts"]):
        ...
        grade = {
            ...
            "warm": ",eq=contrast=1.03:saturation=1.05,colorbalance=rs=0.04:bs=-0.03",
        }[cut["grade"]]
        source_duration = cut["end"] - cut["start"]
        vf = (
            f"[{video_index}:v]trim=duration={source_duration},setpts=(PTS-STARTPTS)/{cut['speed']},"
            f"{_fit(width, height, fps)}{grade},tpad=stop_mode=clone:stop_duration=1,"
            f"trim=duration={length}[v]"
        )
        af = (
            f"[{audio_index}:a]atrim=duration={source_duration},asetpts=PTS-STARTPTS,"
    ...
    (directory / "segments.ffconcat").write_text(
    ...
    for index, overlay in enumerate(plan["overlays"]):
        ...
        ow, oh = (
            (width, height)
            if overlay["layout"] == "full"
            else (width // 3 // 2 * 2, height // 3 // 2 * 2)
        )
        start, end = overlay["start"], overlay["start"] + overlay["duration"]
        position = "0:0" if overlay["layout"] == "full" else "W-w-16:16"
        graph.append(
            f"[{input_index}:v]trim=duration={overlay['duration']},setpts=PTS-STARTPTS,"
            f"{_fit(ow, oh, fps)},setpts=PTS+{start}/TB[over{index}]"
        )
        graph.append(
            f"[{video_label}][over{index}]overlay={position}:eof_action=pass:repeatlast=0:"
    ...
    if narration:
        asset = assets[narration["asset_id"]]
        args += ["-ss", str(narration["source_start"]), *_input(Path(asset["path"]))]
        fade_out = max(0, narration["duration"] - 0.03)
        delay = round(narration["start"] * 1000)
        graph.append(
            f"[{input_index}:a]atrim=duration={narration['duration']},asetpts=PTS-STARTPTS,"
            "aresample=48000,aformat=sample_fmts=fltp:channel_layouts=stereo,"
            f"volume={narration['volume']},afade=t=in:d=0.03,"
            f"afade=t=out:st={fade_out}:d=0.03,adelay={delay}|{delay},"
            f"apad,atrim=duration={duration}[narr]"
        )
        graph.append(
            "[0:a][narr]amix=inputs=2:duration=first:normalize=0,"
            "alimiter=limit=0.95:level=0:latency=1[program]"
        )
        audio_label = "program"
        input_index += 1
    ...
    if music:
        ...
        if music["duck"]:
            graph.append(
                f"[{audio_label}]asplit[voice][side];[bed][side]sidechaincompress="
            ...
            graph.append(
                f"[{audio_label}][bed]amix=inputs=2:duration=first:normalize=0,"
                "alimiter=limit=0.95:level=0:latency=1[a]"
            )
        audio_label = "a"

    captions = _caption_events(plan, transcripts) if plan["captions"] else []
    (directory / "captions.json").write_text(
        json.dumps(captions, ensure_ascii=False, indent=2)
    )
    output = directory / "render.mp4"
    if plan["titles"] or captions:
        layer = _text_layer(
            plan["titles"], captions, duration, width, height, fps, directory
        )
        args += _input(layer)
        graph.append(
            f"[{video_label}][{input_index}:v]overlay=0:0:"
    ...
    args += [
        "-map",
        "0:v" if video_label == "0:v" else f"[{video_label}]",
        "-map",
        "0:a" if audio_label == "0:a" else f"[{audio_label}]",
    ]
    args += ["-c:v", "copy"] if video_label == "0:v" else _video(preview)
    args += [
        "-c:a",
        "aac" if audio_label != "0:a" else "copy",
        "-t",
        str(duration),
        "-movflags",
        "+faststart",
